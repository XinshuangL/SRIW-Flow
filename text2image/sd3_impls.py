...
class Denoiser(torch.nn.Module):
    """Helper for denoising the latent"""
...
def sample_euler(model, x, sigmas, conditioning):
    """Implements Algorithm 2 (Euler steps) from Karras et al. (2022)."""
    s_in = x.new_ones([x.shape[0]])
    for i in tqdm(range(len(sigmas) - 1)):
        sigma_hat = sigmas[i]
        denoised = model(x, sigma_hat * s_in, conditioning)
...
@torch.no_grad()
@torch.autocast("cuda", dtype=torch.float16)
def sample_dpmpp_2m(model, x, sigmas, conditioning):
    """DPM-Solver++(2M)."""
    s_in = x.new_ones([x.shape[0]])
    ...
    old_denoised = None
    for i in tqdm(range(len(sigmas) - 1)):
        denoised = model(x, sigmas[i] * s_in, conditioning)
